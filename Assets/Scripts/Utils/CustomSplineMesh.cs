...
namespace Roots.Utils
{
    public class CustomSplineMesh
    {
        ...
        public static void Extrude<T>(IReadOnlyList<T> splines, Mesh mesh, float radius, IReadOnlyList<SplineData<float>> radiusData, int sides, float segmentsPerUnit, bool capped, float2 range) where T : ISpline
        {
            mesh.Clear();
            ...
            for (int i = 0; i < splines.Count; ++i)
            {
                ...
                settings[i] = new Settings(sides, segments, capped, spline.Closed, range, radius, radiusData[i]);
                ...
                where TIndexType : struct
        {
            ...
            var radiusData = settings.radiusData;
            ...
            {
                var offset = math.lerp(range.x, range.y, i / (segments - 1f));
                
                
                // var scaleOffset = (i / (segments - 1f)) * Random.Range(.85f, 1.15f);
                var scaleOffset = (i / (segments - 1f));
                
                var realRadius = radiusData.Count == 0 ?
                    radiusData.DefaultValue :
                    radiusData.Evaluate(spline, scaleOffset, PathIndexUnit.Normalized, new LerpFloat());
                
                ExtrudeRing(spline,offset, vertices, vertexArrayOffset + i * sides, sides, realRadius);
            }
        ...
        struct Settings
        {
            ...
            public SplineData<float> radiusData { get; private set; }

            public Settings(int sides, int segments, bool capped, bool closed, float2 range, float radius, SplineData<float> radiusData = default)
            {
