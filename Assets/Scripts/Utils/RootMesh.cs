...
public static class RootMesh
{
    ...
    /// do not want to use the vertex layout provided by <see cref="RootMesh"/>."/>.
    /// </summary>
    ...
    struct Settings
    {
        ...
        public float2 scaleRange { get; private set; }

        public Settings(int sides, int segments, bool capped, bool closed, float2 range, float radius, float2 scaleRange)
        {
    ...
    {
        var settings = new Settings(sides, segments, capped, closed, range, 1f, new float2(1, 1));
    ...
    {
        var settings = new Settings(sides, segments, capped, spline.Closed, range, radius, new float2(1, 1));
        ...
        if (indexFormat == IndexFormat.UInt16)
        {
            ...
            Extrude(spline, vertices, indices, radius, sides, segments, capped, range, new float2(1, 1));
        }
        else
        {
    ...
    /// </param>
    /// <typeparam name="T">A type implementing ISpline.</typeparam>
    public static void Extrude<T>(IReadOnlyList<T> splines, Mesh mesh, float radius, int sides, float segmentsPerUnit, bool capped, float2 range, float2 scaleRange) where T : ISpline
    {
        mesh.Clear();
        var meshDataArray = Mesh.AllocateWritableMeshData(1);
        var data = meshDataArray[0];
        ...
        for (int i = 0; i < splines.Count; ++i)
        {
            ...
            settings[i] = new Settings(sides, segments, capped, spline.Closed, range, radius, scaleRange);
    ...
    public static void Extrude<TSplineType, TVertexType, TIndexType>(
        ...
        int segments,
        bool capped,
        float2 range, float2 scaleRange)
        where TSplineType : ISpline
        where TVertexType : struct, ISplineVertexData
        where TIndexType : struct
    {
        Extrude(spline, vertices, indices, new Settings(sides, segments, capped, spline.Closed, range, radius, scaleRange));
    }
        ...
        where TIndexType : struct
    {
        ...
        {
            var a = i / (segments - 1f);
            var t = math.lerp(range.x, range.y, a);
            var s = math.lerp(scaleRange.x, scaleRange.y, a);
            var variation = 1 + Mathf.Sin(i * Mathf.PI * .15f) * .1f;
            ExtrudeRing(spline, t, vertices, vertexArrayOffset + i * sides, sides, radius * s * variation);
        }
        ...
        if (capped)
        {
            ...
            ExtrudeRing(spline, rng.x, vertices, capVertexStart, sides, radius * scaleRange.x);
            ExtrudeRing(spline, rng.y, vertices, endCapVertexStart, sides, radius * scaleRange.y);
